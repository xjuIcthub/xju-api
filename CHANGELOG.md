...
- **Codex / Claude 双 Provider 号池** —— 公共号池新建入口拆为 `CPA (Codex)`、`CPA (Claude)`、`go-pool` 三种；Provider 与构建模式写入 registry/provision 契约，Claude 仅允许公共 CPA 池并只开放 Anthropic OAuth 登录导入。新渠道按 Provider 从 CLIProxyAPI 拉取 Codex 或 Claude 模型目录，杜绝 Claude 池继承 GPT 模型。「所有号池」用六瓣 OpenAI 标识区分 Codex / GPT / OpenAI 池、用官方橙色 Claude 标识区分 Claude 池；Claude 保留 Codex 专属订阅、Wham 额度与重置券的能力隔离，但新增逐账号无推理验活。
- **统一 Provider 用量看板** —— `quota_data` 新增 Provider 维度，旧数据兼容归入 Codex；缓存合并、落库聚合、用户/管理员模型统计与 Flow 查询均支持 `全部 / Codex / Claude`。独立「概览（Claude）/ 数据看板（Claude）」已移除，模型调用分析、分流和用户统计在各自右上工具栏切换 Provider；「全部」由服务端按原分组直接汇总两者，各号池继续使用独立 GroupRatio 计价。
